...
Vector MPU6050::readRawAccel_zzl(void)
{
...
Vector MPU6050::readRawGyro_zzl(void)
{
	Wire.beginTransmission(0x68);
#if ARDUINO >= 100
...
Vector_accel MPU6050::calibrate_accel_pitch_and_roll(uint8_t samples)
{
    float sumPitch = 0;
    float sumRoll = 0;
	...
	double roll = atan(ra.YAxis / sqrt(ra.XAxis * ra.XAxis + ra.ZAxis * ra.ZAxis)) * RAD_TO_DEG;
	double pitch = atan2(-ra.XAxis, ra.ZAxis) * RAD_TO_DEG;
	sumPitch += pitch;
	sumRoll += roll;
	delay(5);
	}

	zzl.pitch = sumPitch / samples;
	zzl.roll = sumRoll / samples;
	return zzl;
}

Vector MPU6050::calibrateGyro_zzl(uint8_t samples)
{
    // Reset values
    float sumX = 0;
    float sumY = 0;
