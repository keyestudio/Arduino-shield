...
class MPU6050
{
	...
	Vector_accel calibrate_accel_pitch_and_roll(uint8_t samples = 50);
	Vector calibrateGyro_zzl(uint8_t samples = 50);
	void calibrateGyro(uint8_t samples = 50);
	void setThreshold(uint8_t multiple = 1);
	uint8_t getThreshold(void);

	Vector readRawGyro(void);
	Vector readRawGyro_zzl(void);
	Vector readNormalizeGyro(void);

	Vector readRawAccel(void);
	Vector readRawAccel_zzl(void);
